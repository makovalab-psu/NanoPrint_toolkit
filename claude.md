...
| `^u` | Relationship whose inputs are already Uncalled4-aligned | `^u SampleName Treatment_uncalled4.bam Control_uncalled4.bam` |
...
Paths come from `RAW_PATHS` (generated by CONFIG.sh from `^r` and `^u` lines — absolute or relative paths):
...
`^u` is the exception: it is declared, not detected. Those inputs are already
Uncalled4-aligned, CONFIG.sh records them in `UNCALLED4_PATHS`, and phases 0 and 1
are skipped entirely for them (see the `^u` dev note at the end of this file).
...
- `uncalled4_align`: filtered_alignments BAM + pod5 → `data/uncalled4/{genome}/{sample}.bam` (BAM, all strands)
  (BAM must have dorado mv/ts tags; Map_reads.sh preserves them via samtools fastq -T + minimap2 -y)
...
**Uncalled4 — `--bam-in` needs an aligned BAM WITH dorado mv tags preserved (js4016, 2026-06-23):**
`uncalled4 align --bam-in` requires BOTH a reference-aligned BAM (with `@SQ` header) AND the
dorado `mv/ts/pi/sp/ns` tags. Two errors were encountered while finding the right fix:
- `Counter({'Missing moves': N})` → BAM aligned but mv tags stripped (minimap2 without `-y`)
- `pysam ValueError: file has no sequences defined` → unaligned dorado BAM passed (no @SQ)

Root cause: `Map_reads.sh` was calling `samtools fastq` (no `-T` flag) and `minimap2` (no `-y`),
silently dropping the dorado tags during alignment. Fix: for BAM input, pipe:
```bash
samtools fastq -T "mv,ts,pi,sp,ns" dorado.bam | minimap2 -y -a -x lr:hq ref.fa -
```
`-T` embeds tags in FASTQ comment fields; `-y` in minimap2 writes them back into the SAM output.
Tags then survive `filter_alignments` (samtools view preserves all BAM tags by default).
Input to `uncalled4_align` is `data/filtered_alignments/{genome}/{raw_sample}.bam` (unchanged).
Also fixed bug in `Map_reads.sh`: intermediate FASTQ was written to `${INPUT_FILE}.bam` (wrong
path/extension); now uses a direct pipe — no temp FASTQ file needed.
...
### `^u`: inputs that are already Uncalled4-aligned (js4022, Sep 2026)

A BAM that has been through `uncalled4 align` had no way into the workflow. Put on an
`^r` line it is treated as ordinary reads: `map_reads` sends it through
`samtools fastq -T <allowlist> | minimap2`, which drops every DTW tag — the reason the
signal alignment was run at all. The README said the opposite ("point the `^r` line at the
Uncalled4 BAM … without re-running Phase 0"), which was never true.

js4026 and js4028 worked around it by **staging**: the BAM was symlinked into
`data/filtered_alignments/{genome}/` and `data/uncalled4/{genome}/`, so Snakemake saw
those rule outputs already present and pruned phases 0 and 1, with `--rerun-triggers mtime`
to stop it deciding to rebuild them. That needed a `01_*.sbatch` per project, a dry-run
guard against `map_reads`/`uncalled4_align` appearing, and hand-written target lists —
`rule all` gated the signal targets on `has_pod5()`, so a bare `snakemake` silently
skipped every phase 2b/3b/4b output.

**`^u` replaces the staging.** It takes the same three fields as `^r`:

```
^u	Sample	Treatment_uncalled4.bam	Control_uncalled4.bam
```

- CONFIG.sh parses it exactly like `^r` (so `RAW_PATHS`, `SAMPLES` and `RELATIONSHIPS`
  are unchanged) and additionally writes `UNCALLED4_PATHS = {raw_sample: path}`.
- It rejects a sample declared on both an `^r` and a `^u` line, and a `^u` path that is
  not a `.bam`. A missing file is a warning, since the Snakefile is often generated
  before the data is staged. A `^u` BAM with no index gets one — `uncalled4 convert`
  needs it, and this is the same reasoning as the `samtools faidx` call above it.
- `phase0_pod5_processing.smk` gains `has_uncalled4()`, `has_signal()` (pod5 *or* `^u`,
  which is what phases 2b/3b/4b actually require), `uncalled4_bam()`/`uncalled4_bai()`
  and `igv_sources()`.
- `uncalled4_convert_tsv` and `find_perbase_bam` call `uncalled4_bam()`. **Returning the
  supplied path verbatim is the whole mechanism**: `data/uncalled4/{genome}/{sample}.bam`
  is never requested for that sample, so Snakemake never instantiates `uncalled4_align`,
  and therefore nothing upstream of it either. No staging, no symlinks, no ruleorder
  against `filter_alignments`, nothing to keep in step.
- `rule all` now gates the signal targets on `has_signal()` instead of `has_pod5()`, so a
  bare `snakemake` builds them for `^u` samples.

**QC: the supplied BAM is the filtered alignment.** It is one — MAPQ ≥ 20, no secondary
or supplementary records — and the pre-filter reads no longer exist, so there is nothing
to put in the `aligned_reads` half. `alignment_stats_uncalled4` (guarded by
`if UNCALLED4_PATHS:`, constrained to those sample names, `ruleorder` above
`alignment_stats`) writes the filtered stats/flagstat files into
`data/filtered_alignments/{genome}/` where the `histograms` rule reads them, and the
`Raw_alignment` column reads `NA`. `rule all` asks only for the `filtered_alignments`
histograms and IGV exports for these samples.

`Alignment_stats.sh` gained the two flags this needs: `-a` is now optional (without it
the raw column is `NA`), and `-d <dir>` places the per-BAM stats/flagstat files somewhere
other than next to the BAM — necessary because a `^u` BAM lives outside the `data/` tree
it is reported under.

**Mixing works.** `^r` and `^u` relationships can sit in one CONFIG; each sample follows
its own route. Verified on a synthetic workspace (2 `^u` samples + 2 `^r` BAM samples,
snakemake 9.23.1 dry run):

- `^u` only: 56 jobs, none of them `dorado_basecall`, `map_reads`, `filter_alignments` or
  `uncalled4_align`; `perbase_error`, `uncalled4_convert_tsv` and
  `alignment_stats_uncalled4` all read the supplied path.
- Mixed: `map_reads` and `filter_alignments` appear twice — for the `^r` pair only —
  which also get both histogram sets, while the `^u` pair gets the filtered set and the
  whole signal chain.
