...
- `uncalled4_align`: basecalled BAM + pod5 → `data/uncalled4/{genome}/{sample}.bam` (BAM, all strands)
  (input is `data/basecalled/`, NOT `filtered_alignments` — minimap2 strips the dorado mv tag)
...
data/basecalled/{sample}.bam               ← has dorado mv tag (move table)
    ...
    └─→ uncalled4_align (phase 0)          ← must use basecalled BAM (not filtered_alignments!)
...
# minimap2 strips the dorado mv (move table) tag → uncalled4 reports "moves missing" for all reads.
# uncalled4 performs its own internal alignment to the reference genome.
uncalled4 align --bam-in <basecalled.bam> --ref <fa> --reads <pod5> -p <threads> -o <out.bam>
...
**Uncalled4 — `--bam-in` must be the dorado BAM, NOT minimap2 output (js4016, 2026-06-23):**
`uncalled4 align --bam-in <filtered_alignments.bam>` fails with `Counter({'Missing moves': N})`
and produces an empty BAM. Root cause: minimap2 strips the dorado `mv` (move table) BAM tag
when it re-aligns reads; uncalled4 needs that tag to do DTW signal alignment. Fix: pass
`data/basecalled/{sample}.bam` (the raw dorado output with `--emit-moves` intact) as `--bam-in`.
uncalled4 then aligns the reads internally using minimap2, independent of the `filtered_alignments`
BAM. Changed in `phase0_pod5_processing.smk`: `uncalled4_align` rule input `bam` now uses
`data/basecalled/{raw_sample}.bam` instead of `data/filtered_alignments/{genome}/{raw_sample}.bam`.
